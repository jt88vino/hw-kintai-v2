...
(async()=>{
 ...
 // Month summary: totals across members, ranking against 80H, and the split by business.
 { const s=api.summarizeMonth({'田中':{totalMinutes:165,daysCount:1,paidTransportDays:1,categoryMinutes:{'ホームワイン':60,'アカデミー':105}},'橋本':{totalMinutes:4800,daysCount:12,paidTransportDays:0,categoryMinutes:{}}},['田中','橋本','喜多'],{'田中':800});
   assert.equal(s.totalMinutes,4965);assert.equal(s.activeCount,2);assert.equal(s.memberCount,3);assert.equal(s.totalDays,13);assert.equal(s.totalTransport,800);
   assert.equal(JSON.stringify(s.rows.map(r=>r.name)),JSON.stringify(['橋本','田中','喜多']));assert.equal(s.rows[0].over,true);assert.equal(s.rows[1].ratio,3);assert.equal(s.rows[2].minutes,0);assert.equal(JSON.stringify(s.overNames),JSON.stringify(['橋本']));assert.equal(JSON.stringify(s.idleNames),JSON.stringify(['喜多']));
   assert.equal(s.categoryTotal,165);assert.equal(s.categories.find(c=>c.category==='アカデミー').share,64);assert.equal(s.categories.find(c=>c.category==='その他（WT業務）').minutes,0);
   const empty=api.summarizeMonth({},[],{});assert.equal(empty.totalMinutes,0);assert.equal(empty.categoryTotal,0);assert.equal(empty.rows.length,0); }
