...
const props=new Map(),reads=[],cache=new Map();
...
let data=ctx.readData_({},sheet,{scope:'recent',fresh:'1'});assert.equal(data.schemaVersion,5);assert.equal(data.logs.length,52);assert(data.logs.some(l=>l.id==='old'));assert(data.logs.some(l=>l.id==='open'));assert.equal(data.months.length,84);
reads.length=0;data=ctx.readData_({},sheet,{scope:'recent',fresh:'1'});assert.equal(reads.reduce((n,r)=>n+r.count,0),52,'warm initial read bounded to recent plus status/session');
reads.length=0;data=ctx.readData_({},sheet,{scope:'recent'});assert.equal(reads.length,0,'warm snapshot skips all sheet ranges');
...
rows.push(row('通常','出勤','2027-01-01 09:00:00','new'));reads.length=0;data=ctx.readData_({},sheet,{scope:'recent',fresh:'1'});assert(data.logs.some(l=>l.id==='new'));assert(reads.reduce((n,r)=>n+r.count,0)<60,'append indexed from new rows only');
// Direct edit trigger and app deletion invalidate row references, including older statuses.
rows[1][2]='出勤';ctx.attendanceSheetChanged({});data=ctx.readData_({},sheet,{scope:'recent',fresh:'1'});assert.equal(data.logs.find(l=>l.id==='old').type,'出勤');
rows.splice(1,1);ctx.invalidateAttendanceIndex_();data=ctx.readData_({},sheet,{scope:'recent',fresh:'1'});assert(!data.logs.some(l=>l.id==='old'));assert(data.logs.some(l=>l.id==='open'));
